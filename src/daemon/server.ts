...
import { DEFAULT_PORT_RANGE, STATE_FILE } from '../shared/types';
import {
  type Learning,
  type TimelineEvent,
  appendLearning,
...
import { type Skill, loadAllSkills } from './skill-loader';
...
function randomPort(): number {
  return (
    Math.floor(Math.random() * (DEFAULT_PORT_RANGE.max - DEFAULT_PORT_RANGE.min)) +
    DEFAULT_PORT_RANGE.min
  );
}
      ...
      return req
        ...
        .then(async (body: { skill: string; args?: string[] }) => {
          ...
          if (!skillObj) {
            return Response.json({
              ok: false,
              error: `Unknown skill: ${skill}. Run 'mia help' for available commands.`,
            });
          }
