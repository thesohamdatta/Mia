...
export interface Skill {
  ...
  execute: (args: string[], token: string) => Promise<{ ok: boolean; output?: string; error?: string }>;
}
...
function createBuiltinExecutor(skillName: string) {
  return async (args: string[], token: string) => {
...
async function loadSkillFromDir(skillDir: string): Promise<Skill | null> {
  ...
  let executor: (args: string[], token: string) => Promise<{ ok: boolean; output?: string; error?: string }>;
