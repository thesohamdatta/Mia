...
export function readLearnings(slug: string, limit: number = 50): Learning[] {
...
export function readTimeline(slug: string, limit: number = 30): TimelineEvent[] {
  const path = timelinePath(slug);
  if (!existsSync(path)) return [];
  const lines = readFileSync(path, "utf-8").trim().split("\n").filter(Boolean);
  return lines.slice(-limit).map(l => JSON.parse(l) as TimelineEvent).reverse();
}
...
export function listCheckpoints(slug: string): string[] {
  ...
  return readdirSync(dir).filter((f: string) => f.endsWith(".md")).sort().reverse();
}
...
export function ensureMemory(): void {
  if (!existsSync(memoryPath())) {
    writeFileSync(memoryPath(), "# MIA Long-Term Memory\n\n_Curated wisdom, distilled from daily notes._\n", "utf-8");
  }
}
